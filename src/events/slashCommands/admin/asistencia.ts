import { SlashCommandBuilder, CommandInteraction, Client, GuildMember, EmbedBuilder, VoiceState } from 'discord.js';
...
export default {
    ...
    async execute(interaction: CommandInteraction, client: Client) {
        ...
        if (!hasPermission) {
            await interaction.reply({ content: '<:no:1288631410558767156> No tienes permisos para usar este comando.', ephemeral: true });
            return;
        }

        const voiceChannel = member.voice.channel;
        if (!voiceChannel) {
            await interaction.reply({ content: '<:no:1288631410558767156> Debes estar en un canal de voz para usar este comando.', ephemeral: true });
            return;
        }
        ...
        if (tracking && trackedChannelId === voiceChannel.id) {
            ...
            const voiceStateUpdateHandler = (_oldState: VoiceState, newState: VoiceState) => {
                ...
                if (trackedChannel?.isVoiceBased() && trackedChannel.members.size === 0) {
                    tracking = false;
                    trackedChannelId = null;
