...
export default {
    data: new SlashCommandBuilder()
        .setName('ins_motorsport')
        .setDescription('Inscriptor del MotorSport')
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
    // MARK: Mensaje de inscripción
    async execute(interaction, client) {
        // Marca la interacción como respondida no como pensando
        await interaction.reply({
            content: '<:si:1288631406452412428> Inscriptor del MotorSport enviado',
            flags: MessageFlags.Ephemeral
        })
        const container = new ContainerBuilder()
        const title = new TextDisplayBuilder()
            .setContent('# <a:flecha:1290411623802208257> ¡Bienvenid@ a la categoría de MotorSport!')
        ...
        const introduccion = new TextDisplayBuilder()
            .setContent(
                `## <:chat_ind:1288628721842130976> Configuración de la categoría \n> La categoría es totalemte configurable por el rol <@&${discordConfig.mentions.asociationsRoleIds.motorsport.coord}>`
            )
        const miniatura = new ThumbnailBuilder({
            media: {
                url: 'https://media.discordapp.net/attachments/1288785370783420416/1394279661269418147/UVigoMotorsportGaliciaLogo_1.png?ex=68763b9f&is=6874ea1f&hm=298df54cefa85cf8391e098f922a8f2238597fac39c903c2e911f383b1febae7&=&format=webp&quality=lossless&width=570&height=856'
            }
        })
        ...
        const miembros = new TextDisplayBuilder()
            .setContent('## <:us:1288631396364976128> Miembros de MotorSport')
        ...
        const userSelect = new UserSelectMenuBuilder()
            .setCustomId('ins_motorsport_user_select')
        ...
        const nuevosMiembrosButton = new ButtonBuilder()
            .setCustomId('ins_motorsport_nuevos')
        ...
        const eliminarMiembrosButton = new ButtonBuilder()
            .setCustomId('ins_motorsport_eliminar')
        ...
        container.addTextDisplayComponents(
            new TextDisplayBuilder()
                .setContent('-# <:dai:1288623399672741930>  Delegación de alumnado de industriales - UVigo · **Categoría de MotorSport**')
        )
    ...
    // MARK: Lógica
    /**
     * Maneja las interacciones de componentes (botones y menús de selección) del comando ins_motorsport
    ...
    async handleComponentInteraction(interaction, client) {
        // Verificar permisos: solo coordinador del Motorsport y administradores
        const member = interaction.member;
        const isAdmin = member.permissions.has(PermissionFlagsBits.Administrator);
        const isMotorsportCoordinator = member.roles.cache.has(discordConfig.mentions.asociationsRoleIds.motorsport.coord);

        if (!isAdmin && !isMotorsportCoordinator) {
            return await interaction.reply({
                content: '<:no:1288631410558767156> No tienes permisos para usar esta función. Solo el coordinador del Motorsport y los administradores pueden usarla.',
                flags: MessageFlags.Ephemeral
            });
        }
        ...
        if (interaction.isUserSelectMenu() && interaction.customId === 'ins_motorsport_user_select') {
            selectedUser = interaction.values[0]; // Obtener el ID del usuario seleccionado

            // Responder a la interacción del menú de selección
            if (interaction.guild.members.cache.get(selectedUser).roles.cache.has(discordConfig.mentions.asociationsRoleIds.motorsport.member)) { // Verificar si el usuario ya tiene el rol
                await interaction.reply({
                content: `<:verificado:1288628715982553188> Usuario <@${selectedUser}> seleccionado.\n> El usuario **es** miembro del MotorSport.\n > <:salir:1288975442828726374> Presiona **Eliminar Miembro** para eliminarlo.`,
                flags: MessageFlags.Ephemeral
            })
            } else {
                await interaction.reply({
                content: `<:verificado:1288628715982553188> Usuario <@${selectedUser}> seleccionado.\n> El usuario **no** miembro del MotorSport.\n > <:entrar:1288631392070012960> Presiona **Añadir Miembro** para añadirlo.`,
                flags: MessageFlags.Ephemeral
            })
            }
            // Guardar el usuario seleccionado y el ID de la interacción original
            if (!client.tempData) client.tempData = new Map();
            client.tempData.set(`motorsport_selected_user_${interaction.user.id}`, selectedUser);
            client.tempData.set(`motorsport_original_interaction_${interaction.user.id}`, interaction);
            return;
        }
        ...
        if (interaction.isButton()) {
            ...
            if (!client.tempData) client.tempData = new Map();
            selectedUser = client.tempData.get(`motorsport_selected_user_${interaction.user.id}`);
            const originalInteraction = client.tempData.get(`motorsport_original_interaction_${interaction.user.id}`);
            ...
            // ID del rol de miembro del MotorSport desde la configuración
            const memberRoleId = discordConfig.mentions.asociationsRoleIds.motorsport.member;
            ...
            try {
                ...
                if (!role) {
                    return await interaction.reply({
                        content: '<:no:1288631410558767156> No se pudo encontrar el rol de miembro del MotorSport.',
                        flags: MessageFlags.Ephemeral
                    });
                }
                ...
                if (interaction.customId === 'ins_motorsport_nuevos') {
                    // Verificar si ya tiene el rol
                    if (targetMember.roles.cache.has(memberRoleId)) {
                        await interaction.deferUpdate();
                        return await originalInteraction.editReply({
                            content: `<:no:1288631410558767156> El usuario <@${selectedUser}> **ya es miembro** del MotorSport.`
                        });
                    }

                    // Responder inmediatamente indicando que se está promotorsportando
                    await interaction.deferUpdate();
                    ...
                    client.tempData.delete(`motorsport_selected_user_${interaction.user.id}`);
                    client.tempData.delete(`motorsport_original_interaction_${interaction.user.id}`);

                    // Editar la respuesta con el resultado final
                    await originalInteraction.editReply({
                        content: `<:entrar:1288631392070012960> Se ha añadido a <@${selectedUser}> como **miembro del MotorSport**.\n-# La selección se ha limpiado automáticamente. Selecciona otro usuario si nemotorsportitas hacer más acciones.`
                    });
                } else if (interaction.customId === 'ins_motorsport_eliminar') {
                    // Verificar si tiene el rol
                    if (!targetMember.roles.cache.has(memberRoleId)) {
                        await interaction.deferUpdate();
                        return await originalInteraction.editReply({
                            content: `<:no:1288631410558767156> El usuario <@${selectedUser}> **no es miembro** del MotorSport.`
                        });
                    }
                    ...
                    // Editar la respuesta con el resultado final
                    await originalInteraction.editReply({
                        content: `<:salir:1288975442828726374> Se ha eliminado a <@${selectedUser}> como **miembro del MotorSport**.\n-# La selección se ha limpiado automáticamente. Selecciona otro usuario si nemotorsportitas hacer más acciones.`
                    });
                }
            } catch (error) {
                console.error('Error manejando la interacción del MotorSport:', error);
                ...
                try {
                    ...
                    if (originalInteraction) {
                        await originalInteraction.editReply({
                            content: '<:no:1288631410558767156> Hubo un error al promotorsportar la solicitud. Contacta con un administrador.'
                        });
                    }
                } catch (followUpError) {
